...
static const int INPUT_W = 416;  //light model
static const int INPUT_H = 416;
static const int NUM_CLASSES = 2;
const char* INPUT_BLOB_NAME = "input_0";     //1
const char* OUTPUT_BLOB_NAME = "output_0";
// const char* INPUT_BLOB_NAME = "images";  //2
...
cv::Mat static_resize(cv::Mat& img) {
    ...
    cv::Mat re(unpad_h, unpad_w, CV_8UC3);//(416,234)
    cv::resize(img, re, re.size());
    cv::Mat out(INPUT_H, INPUT_W, CV_8UC3, cv::Scalar(114, 114, 114));//out:padding(416，416)
...
int main(int argc, char** argv) {
    ...
    if (argc == 4 && std::string(argv[2]) == "-i") {//四个参数：./exe engine_file_path -i input_image_path
